...
Чтобы все выше описанное имело хоть какое-то значение, надо включить модуль с помощью `HAL_USE_GPT` в `halconf.h` и включить выбранный таймер с помощью `STM32_GPT_USE_TIMx` в `mcuconf.h`. Если не понятно о чем реч, смотрим [сюда](Basics.md).
...
Поле `frequency` - частота в герцах. Писать можно любое целое число, которое можно получить с помощью 16-битного предделителя исходя из частоты шины. Если, вдруг, предыдущее предложение было предельно ясно, следующий фрагмент можно пропустить.
