...
`i2cGetErrors()` возвращает флаги ошибок, аргументом является указатель на драйвер, например, `&I2CD1`. Флаги организованы в виде маски, то есть каждый бит отвечает за свою ошибку (см. таблицу). Если все биты нули, значит ошибок нет. В принципе этим не обязательно пользоваться, но, можно например что-нибудь сделать, если не пришел `ACK` бит или есть проблемы на линн. 
...
существует функция повторного старта. Идея заключается в том, что сначала выполняется старт, с указанием адреса на запись, передается информация (например 
адрес интересующего регистра), а затем происходит еще один старт без операции остановки. Второй раз указывается адрес того же устройства, но уже на чтение. В этом случае устройство будет передавать информацию из указанного ранее регистра. Не смотря на то, что это не является частью стандарта, подобный подход применяется
практически во всех современных устройствах. В связи с этим в данной функции повторный старт присутсвует по умолчанию, что влияет на ее аргументы.
