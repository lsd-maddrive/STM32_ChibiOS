...
PAL - драйвер, управляющий портами ввода-вывода, также известными как GPIO. Все функции данного модуля начинаются с префикса pal, что позволяет пользоваться автозаполнением (ctrl + пробел при неполном введении функции в Eclipse) для просмотра всех возможных функций.

Чтобы включить дравйвер, необходимо в halconf.h установить HAL_USE_PAL в состояние TRUE.
...
Для того, чтобы сделать что-либо с ногами контроллера, нужно выбрать соответствующий режим их работы. Для этого есть несколько функций:
  ...
  - palSetPadMode(port, pad, mode) - также выбирает режим для одной линии.
...
Режим определяет поведение ноги. Например, будет она на вход, выход или будет обеспечивать работу какого-либо модуля. Для удобного выбора режима в ChibiOS есть несколько дефайнов:
  ...
  - PAL_MODE_RESET - устанавливает режим линии в исходное для используемой платформы состояние;
  ...
  - PAL_MODE_OUTPUT_PUSHPULL - нога на выход в режиме PUSHPULL;
  - PAL_MODE_OUTPUT_OPENDRAIN - нога на выход в режиме OPENDRAIN;
...
У некоторых платформ есть дополнительные режимы. Например, в случае контроллеров stm32 есть настройка скорости порта:
...
Подробнее про эти режимы надо искать в документации на конкретный контроллер.
...
Начиная с CnibiOS18 внешние прерывания переехали из модуля ext в pal.
...
Функция обработчик должна принима в себя один аргумент - указатель на передаваемый аргумент. Например:
