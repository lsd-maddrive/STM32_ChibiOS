...
> Сройлер - в конце есть инструкция для ленивых =)
...
В нее мы будем складывать все наши собранные либы бинарники. Также рядом создайте директорию `ext_repos`, в которую будем класть исходники перед сборкой.
...
> Почитайте про опцию `-j` у команды `make`. Я вызывал с тайо опцией, потому что у меня 4 потока на процессор.
...
> whoami - утилита, которая отображает имя пользователя, а в команде мы используем кавычки, чтобы результат выполнения уутилиты подставился в качестве стрпоки в команде.
...
Ошибка сообщает, что не может найти библиотеку, что достаточно распространено, есть два пути:
...
> Тут используется не просто перенаправление в файл, а еще и команда `tee`. Сделано это, чтобы выполнить с `sudo` команду, наподобии этой `echo "$HOME/local_libs/lib" > /etc/ld.so.conf.d/user_libs.conf`
