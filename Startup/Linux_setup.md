...
О том, как создавать проект написано [здесь](../ProjectTemplates/New_project.md). После создания проекта переходим в папку и вызывает сборку:
```bash
make -j4
```
...
Тянем архив с [облака](https://disk.yandex.ru/d/awki7AnoKNxiuA), распаковываем:
```bash
